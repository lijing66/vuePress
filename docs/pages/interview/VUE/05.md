...
vue 是数据驱动视图的框架： 数据变化之后视图是随之变化的。 但是，这个过程是异步的。nextTick 这个api就是用来实现这个异步更新的过程的。
...
如果想要在修改数据后立刻得到更新后的`DOM`结构，可以使用`Vue.nextTick()`。例如：新闻列表中数据保存在数组中，如果增加一条数据之后，希望得到新闻列表区域的高度，此时就可以把这个获取高度的动作放在nextTick中执行。
