...
react中的hooks有一些使用限制，例如，不能写在if内部，也不能嵌套者写。这里有条件渲染。

react的状态和UI没有对应关系，必须按顺序走。

vue中的hooks就没有这个问题。Ui和状态变化是一一对应的，是否写在if中没有关系。

react是完全是运行时的库，vue可以在编译时

hooks和class都是运行在react源码底层运行时之上的API，它们的地位是平等的（厨师做了两盘菜，看看你喜欢哪个）
