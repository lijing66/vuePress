<template><div><h1 id="fiber架构的理解-解决了什么问题" tabindex="-1"><a class="header-anchor" href="#fiber架构的理解-解决了什么问题" aria-hidden="true">#</a> Fiber架构的理解？解决了什么问题？</h1>
<h2 id="一、问题" tabindex="-1"><a class="header-anchor" href="#一、问题" aria-hidden="true">#</a> 一、问题</h2>
<p><code v-pre>JavaScript</code>引擎和页面渲染引擎两个线程是互斥的，当其中一个线程执行时，另一个线程只能挂起等待</p>
<p>如果 <code v-pre>JavaScript</code> 线程长时间地占用了主线程，那么渲染层面的更新就不得不长时间地等待，界面长时间不更新，会导致页面响应度变差，用户可能会感觉到卡顿</p>
<p>而这也正是 <code v-pre>React 15</code> 的 <code v-pre>Stack Reconciler</code>所面临的问题，当 <code v-pre>React</code>在渲染组件时，从开始到渲染完成整个过程是一气呵成的，无法中断</p>
<p>如果组件较大，那么<code v-pre>js</code>线程会一直执行，然后等到整棵<code v-pre>VDOM</code>树计算完成后，才会交给渲染的线程</p>
<p>这就会导致一些用户交互、动画等任务无法立即得到处理，导致卡顿的情况</p>
<p><img src="https://static.vue-js.com/5eb3a850-ed24-11eb-ab90-d9ae814b240d.png" alt=""></p>
<h2 id="二、是什么" tabindex="-1"><a class="header-anchor" href="#二、是什么" aria-hidden="true">#</a> 二、是什么</h2>
<p>React Fiber 是 Facebook 花费两年余时间对 React 做出的一个重大改变与优化，是对 React 核心算法的一次重新实现。从Facebook在 React Conf 2017 会议上确认，React Fiber 在React 16 版本发布</p>
<p>在<code v-pre>react</code>中，主要做了以下的操作：</p>
<ul>
...
<p>实现的上述方式的是<code v-pre>requestIdleCallback</code>方法</p>
<p><code v-pre>window.requestIdleCallback()</code>方法将在浏览器的空闲时段内调用的函数排队。这使开发者能够在主事件循环上执行后台和低优先级工作，而不会影响延迟关键事件，如动画和输入响应</p>
...
<ul>
<li>https://juejin.cn/post/6926432527980691470</li>
<li>https://zhuanlan.zhihu.com/p/137234573</li>
</ul>
</div></template>
