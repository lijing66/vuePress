 ...
 * @Description: 
-->
<h1 id="说说-http-常见的状态码有哪些-适用场景" tabindex="-1"><a class="header-anchor" href="#说说-http-常见的状态码有哪些-适用场景" aria-hidden="true">#</a> 说说 HTTP 常见的状态码有哪些，适用场景？</h1>
...
<p>201 Created 请求已经被实现，⽽且有⼀个新的资源已经依据请求的需要⽽建⽴ 通常是在 POST 请求，或是某些 PUT 请求之后创建了内容, 进行的返回的响应</p>
<p>202 Accepted 请求服务器已接受，但是尚未处理，不保证完成请求 适合异步任务或者说需要处理时间比较长的请求，避免 HTTP 连接一直占用</p>
<p>204 No content 表示请求成功，但响应报⽂不含实体的主体部分</p>
<p>206 Partial Content 进⾏的是范围请求, 表示服务器已经成功处理了部分 GET 请求 响应头中会包含获取的内容范围 (常用于分段下载)</p>
<p><strong>重定向（3XX）</strong></p>
<p>状态码 原因短语 说明</p>
<p>301 Moved Permanently 永久性重定向，表示资源已被分配了新的 URL 比如，我们访问 http://www.baidu.com 会跳转到 https://www.baidu.com</p>
<p>302 Found 临时性重定向，表示资源临时被分配了新的 URL, 支持搜索引擎优化 首页, 个人中心, 遇到了需要登录才能操作的内容, 重定向 到 登录页</p>
<p>303 See Other 对于 POST 请求，它表示请求已经被处理，客户端可以接着使用 GET 方法去请求 Location 里的 URI。</p>
<p>304 Not Modified 自从上次请求后，请求的网页内容未修改过。 服务器返回此响应时，不会返回网页内容。(协商缓存)</p>
<p>307 Temporary Redirect 对于 POST 请求，表示请求还没有被处理，客户端应该向 Location 里的 URI 重新发起 POST 请求。</p>
<p>不对请求做额外处理, 正常发送请求, 请求 location 中的 url 地址</p>
<p>因为 post 请求, 是非幂等的, 从 302 中, 细化出了 303 和 307</p>
<p>简而言之:</p>
<ul>
<li>
<p>301 302 307 都是重定向</p>
</li>
<li>
<p>304 协商缓存</p>
</li>
...
<p>状态码 原因短语 说明</p>
<p>400 Bad Request 请求报⽂存在语法错误（传参格式不正确）</p>
<p>401 UnAuthorized 权限认证未通过(没有权限)</p>
<p>403 Forbidden 表示对请求资源的访问被服务器拒绝</p>
<p>404 Not Found 表示在服务器上没有找到请求的资源</p>
<p>408 Request Timeout 客户端请求超时</p>
<p>409 Confict 请求的资源可能引起冲突</p>
<p><strong>服务端错误（5XX）</strong></p>
<p>状态码 原因短语 说明</p>
<p>500 Internal Sever Error 表示服务器端在执⾏请求时发⽣了错误</p>
<p>501 Not Implemented 请求超出服务器能⼒范围，例如服务器不⽀持当前请求所需要的某个功能， 或者请求是服务器不⽀持的某个⽅法</p>
<p>503 Service Unavailable 表明服务器暂时处于超负载或正在停机维护，⽆法处理请求</p>
<p>505 Http Version Not Supported 服务器不⽀持，或者拒绝⽀持在请求中使⽤的 HTTP 版本</p>
...
<p>它由 RFC 2616 规范定义的，并得到 <code v-pre>RFC 2518</code>、<code v-pre>RFC 2817</code>、<code v-pre>RFC 2295</code>、<code v-pre>RFC 2774</code>与 <code v-pre>RFC 4918</code>等规范扩展</p>
<p>简单来讲，<code v-pre>http</code>状态码的作用是服务器告诉客户端当前请求响应的状态，通过状态码就能判断和分析服务器的运行状态</p>
...
<h2 id="三、适用场景" tabindex="-1"><a class="header-anchor" href="#三、适用场景" aria-hidden="true">#</a> 三、适用场景</h2>
<p>下面给出一些状态码的适用场景：</p>
<ul>
<li>
...
<ul>
<li>https://zh.wikipedia.org/wiki/HTTP状态码</li>
<li>https://kebingzao.com/2018/10/05/http-status-code/</li>
</ul>
</div></template>
