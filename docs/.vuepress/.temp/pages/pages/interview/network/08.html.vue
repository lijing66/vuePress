...
<p>三次握手是为了确认双方的接收与发送能力是否正常。</p>
<p>1、第一次握手：客户端给服务器发送一个 SYN 报文。</p>
<p>2、第二次握手：服务器收到 SYN 报文之后，会应答一个 SYN+ACK 报文。</p>
<p>3、第三次握手：客户端收到 SYN+ACK 报文之后，会回应一个 ACK 报文。 服务器收到 ACK 报文之后，三次握手建立完成。</p>
<p>当客户端和服务端通过三次握手建立 TCP 连接进行可靠数据传输后，当数据传送完毕，肯定是要断开 TCP 连接，所以就有了四次挥手。</p>
<p>四次挥手是为了断开连接后「资源」正常释放，双方都可以主动断开连接。</p>
<p>1、第一次挥手：客户端发送一个 FIN 报文，报文中会指定一个序列号。此时客户端处于 FIN_WAIT1 状态。</p>
<p>2、第二次握手：服务端收到 FIN 之后，会发送 ACK 报文，且把客户端的序列号值 + 1 作为 ACK 报文的序列号值，表明已经收到客户端的报文了，此时服务端处于 CLOSE_WAIT 状态。</p>
...
<h3 id="扩展理解" tabindex="-1"><a class="header-anchor" href="#扩展理解" aria-hidden="true">#</a> 扩展理解</h3>
<p>为什么只有三次握手才能确认双方的接受与发送能力是否正常，而两次却不可以：</p>
